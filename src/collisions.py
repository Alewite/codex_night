...
def make_block_zone(x, y, width, height):
    # переводим координаты из размера исходной карты в размер мира
    return pygame.Rect(
        int(x * MAP_SCALE),
        int(y * MAP_SCALE),
        int(width * MAP_SCALE),
...
def rect_collides(rect):
    # проверяем пересечение с каждой закрытой зоной
...
def clamp_to_world(rect):
    # не выпускаем объект за границы мира
    if rect.left < 0:
        rect.left = 0
...
def move_with_collisions(rect, dx, dy, speed):
    ...
    # откатываем движение по x если объект вошел в стену
    ...
    # откатываем движение по y если объект вошел в стену
    if rect_collides(rect):
        rect.y = old_y
        blocked = True
